...
function renderGameDetails(game: Game): void {
  const title = document.getElementById("game-title");
  const image = document.getElementById("game-image") as HTMLImageElement;
  ...
  if (!game || !title || !image || !publisher || !categories || !players || !duration || !mode || !description || !rules || !video || !secondaryImages) return;
